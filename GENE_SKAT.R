...
# LINE <- c("/projects/janssen/RV_Rare_Variant/20140617_2014_Plenge_LT8_DEL_MNe_MN_DAS_BL_MN_AGE_SEX_PC1_PC2","/projects/janssen/RV_Rare_Variant/20140617_2014_Plenge_LT8_DEL_MNe_MN_DAS_BL_MN_AGE_SEX_PC1_PC2/Alt_2014_Plenge.txt","/projects/janssen/RV_Rare_Variant/../ASSOCIATION/PH-PHENOTYPES/LT8_DEL_MNe_MN.txt","C","/projects/janssen/RV_Rare_Variant/20140617_2014_Plenge_LT8_DEL_MNe_MN_DAS_BL_MN_AGE_SEX_PC1_PC2/Cov_w_PCs.txt","DAS_BL,AGE,SEX,PC1,PC2")
...
if ( !identical(sort(G_Names),sort(Samp_Names)) ) { Samp_Names <- intersect(G_Names,Samp_Names) }
...
#############################################
## COMPILE AND SAVE DATA (every 10 iters) ###
#############################################
if (i %% 5 == 0) {
	...
	names(P_SO) <- names(N_VAR) <- names(RO_VAL) <- SET_NAMES
	names(P_SK) <- names(P_BD) <- SET_NAMES[1:4]
	COMPILE <- list(P_SO,P_SK,P_BD,N_VAR,RO_VAL)
	names(COMPILE) <- c("P_SKAT-O","P_SKAT","P_Burden","Num_Vars","Rho_Val")
